object FormPalettes: TFormPalettes
  Left = 0
  Top = 0
  BorderIcons = [biSystemMenu]
  BorderStyle = bsSingle
  Caption = 'Palettes'
  ClientHeight = 320
  ClientWidth = 280
  Color = clBtnFace
  DoubleBuffered = True
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  FormStyle = fsMDIChild
  Visible = True
  TextHeight = 15
  object Label1: TLabel
    Left = 8
    Top = 8
    Width = 18
    Height = 108
    Caption = 'Background Palettes'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Orientation = 900
    Font.Style = []
    ParentFont = False
  end
  object Shape01: TShape
    Left = 32
    Top = 8
    Width = 24
    Height = 24
  end
  object Shape02: TShape
    Left = 60
    Top = 8
    Width = 24
    Height = 24
  end
  object Shape03: TShape
    Left = 88
    Top = 8
    Width = 24
    Height = 24
  end
  object Shape04: TShape
    Left = 116
    Top = 8
    Width = 24
    Height = 24
  end
  object Shape05: TShape
    Left = 32
    Top = 36
    Width = 24
    Height = 24
  end
  object Shape06: TShape
    Left = 60
    Top = 36
    Width = 24
    Height = 24
  end
  object Shape07: TShape
    Left = 88
    Top = 36
    Width = 24
    Height = 24
  end
  object Shape08: TShape
    Left = 116
    Top = 36
    Width = 24
    Height = 24
  end
  object Shape09: TShape
    Left = 32
    Top = 64
    Width = 24
    Height = 24
  end
  object Shape10: TShape
    Left = 60
    Top = 64
    Width = 24
    Height = 24
  end
  object Shape11: TShape
    Left = 88
    Top = 64
    Width = 24
    Height = 24
  end
  object Shape12: TShape
    Left = 116
    Top = 64
    Width = 24
    Height = 24
  end
  object Shape13: TShape
    Left = 32
    Top = 92
    Width = 24
    Height = 24
  end
  object Shape14: TShape
    Left = 60
    Top = 92
    Width = 24
    Height = 24
  end
  object Shape15: TShape
    Left = 88
    Top = 92
    Width = 24
    Height = 24
  end
  object Shape16: TShape
    Left = 116
    Top = 92
    Width = 24
    Height = 24
  end
  object Shape17: TShape
    Left = 32
    Top = 120
    Width = 24
    Height = 24
  end
  object Shape18: TShape
    Left = 60
    Top = 120
    Width = 24
    Height = 24
  end
  object Shape19: TShape
    Left = 88
    Top = 120
    Width = 24
    Height = 24
  end
  object Shape20: TShape
    Left = 116
    Top = 120
    Width = 24
    Height = 24
  end
  object Shape21: TShape
    Left = 32
    Top = 148
    Width = 24
    Height = 24
  end
  object Shape22: TShape
    Left = 60
    Top = 148
    Width = 24
    Height = 24
  end
  object Shape23: TShape
    Left = 88
    Top = 148
    Width = 24
    Height = 24
  end
  object Shape24: TShape
    Left = 116
    Top = 148
    Width = 24
    Height = 24
  end
  object Shape25: TShape
    Left = 32
    Top = 176
    Width = 24
    Height = 24
  end
  object Shape26: TShape
    Left = 60
    Top = 176
    Width = 24
    Height = 24
  end
  object Shape27: TShape
    Left = 88
    Top = 176
    Width = 24
    Height = 24
  end
  object Shape28: TShape
    Left = 116
    Top = 176
    Width = 24
    Height = 24
  end
  object Shape29: TShape
    Left = 32
    Top = 204
    Width = 24
    Height = 24
  end
  object Shape30: TShape
    Left = 60
    Top = 204
    Width = 24
    Height = 24
  end
  object Shape31: TShape
    Left = 88
    Top = 204
    Width = 24
    Height = 24
  end
  object Shape32: TShape
    Left = 116
    Top = 204
    Width = 24
    Height = 24
  end
  object Shape33: TShape
    Left = 192
    Top = 8
    Width = 24
    Height = 24
  end
  object Shape34: TShape
    Left = 220
    Top = 8
    Width = 24
    Height = 24
  end
  object Shape35: TShape
    Left = 248
    Top = 8
    Width = 24
    Height = 24
  end
  object Shape36: TShape
    Left = 192
    Top = 36
    Width = 24
    Height = 24
  end
  object Shape37: TShape
    Left = 220
    Top = 36
    Width = 24
    Height = 24
  end
  object Shape38: TShape
    Left = 248
    Top = 36
    Width = 24
    Height = 24
  end
  object Shape39: TShape
    Left = 192
    Top = 64
    Width = 24
    Height = 24
  end
  object Shape40: TShape
    Left = 220
    Top = 64
    Width = 24
    Height = 24
  end
  object Shape41: TShape
    Left = 248
    Top = 64
    Width = 24
    Height = 24
  end
  object Shape42: TShape
    Left = 192
    Top = 92
    Width = 24
    Height = 24
  end
  object Shape43: TShape
    Left = 220
    Top = 92
    Width = 24
    Height = 24
  end
  object Shape44: TShape
    Left = 248
    Top = 92
    Width = 24
    Height = 24
  end
  object Shape45: TShape
    Left = 192
    Top = 120
    Width = 24
    Height = 24
  end
  object Shape46: TShape
    Left = 220
    Top = 120
    Width = 24
    Height = 24
  end
  object Shape47: TShape
    Left = 248
    Top = 120
    Width = 24
    Height = 24
  end
  object Shape48: TShape
    Left = 192
    Top = 148
    Width = 24
    Height = 24
  end
  object Shape49: TShape
    Left = 220
    Top = 148
    Width = 24
    Height = 24
  end
  object Shape50: TShape
    Left = 248
    Top = 148
    Width = 24
    Height = 24
  end
  object Shape51: TShape
    Left = 192
    Top = 176
    Width = 24
    Height = 24
  end
  object Shape52: TShape
    Left = 220
    Top = 176
    Width = 24
    Height = 24
  end
  object Shape53: TShape
    Left = 248
    Top = 176
    Width = 24
    Height = 24
  end
  object Shape54: TShape
    Left = 192
    Top = 204
    Width = 24
    Height = 24
  end
  object Shape55: TShape
    Left = 220
    Top = 204
    Width = 24
    Height = 24
  end
  object Shape56: TShape
    Left = 248
    Top = 204
    Width = 24
    Height = 24
  end
  object Label2: TLabel
    Left = 168
    Top = 8
    Width = 18
    Height = 79
    Caption = 'Object Palettes'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Segoe UI'
    Font.Orientation = 900
    Font.Style = []
    ParentFont = False
  end
  object ShapeBackground: TShape
    Left = 32
    Top = 264
    Width = 48
    Height = 48
  end
  object ShapeForeground: TShape
    Left = 8
    Top = 240
    Width = 48
    Height = 48
  end
  object Shape00: TShape
    Left = 8
    Top = 296
    Width = 16
    Height = 16
    Brush.Color = clRed
    Brush.Style = bsFDiagonal
    Pen.Color = clRed
    OnMouseDown = Shape00MouseDown
  end
  object GroupBoxEditor: TGroupBox
    Left = 88
    Top = 240
    Width = 184
    Height = 72
    Caption = 'Editor'
    DoubleBuffered = False
    ParentDoubleBuffered = False
    TabOrder = 0
    object Label3: TLabel
      Left = 8
      Top = 19
      Width = 26
      Height = 15
      Caption = 'CGB:'
    end
    object Label4: TLabel
      Left = 91
      Top = 19
      Width = 23
      Height = 15
      Caption = 'CSS:'
    end
    object Label5: TLabel
      Left = 8
      Top = 43
      Width = 10
      Height = 15
      Caption = 'R:'
    end
    object Label6: TLabel
      Left = 66
      Top = 43
      Width = 11
      Height = 15
      Caption = 'G:'
    end
    object Label7: TLabel
      Left = 124
      Top = 43
      Width = 10
      Height = 15
      Caption = 'B:'
    end
    object ShapePicker: TShape
      Left = 162
      Top = 0
      Width = 15
      Height = 15
      DragCursor = crCross
      Shape = stCircle
      OnMouseDown = ShapePickerMouseDown
      OnMouseUp = ShapePickerMouseUp
    end
    object EditCGB: TEdit
      Left = 40
      Top = 16
      Width = 42
      Height = 23
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Courier New'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      Text = 'xABCD'
      OnChange = EditCGBChange
      OnExit = EditExit
    end
    object EditCSS: TEdit
      Left = 120
      Top = 16
      Width = 56
      Height = 23
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Courier New'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      Text = '#ABCDDD'
      OnChange = EditCSSChange
      OnExit = EditExit
    end
    object SpinEditR: TSpinEdit
      Left = 24
      Top = 40
      Width = 36
      Height = 24
      MaxValue = 31
      MinValue = 0
      TabOrder = 2
      Value = 31
      OnChange = SpinEditRGBChange
      OnExit = EditExit
    end
    object SpinEditG: TSpinEdit
      Left = 82
      Top = 40
      Width = 36
      Height = 24
      MaxValue = 31
      MinValue = 0
      TabOrder = 3
      Value = 31
      OnChange = SpinEditRGBChange
      OnExit = EditExit
    end
    object SpinEditB: TSpinEdit
      Left = 140
      Top = 40
      Width = 36
      Height = 24
      MaxValue = 31
      MinValue = 0
      TabOrder = 4
      Value = 31
      OnChange = SpinEditRGBChange
      OnExit = EditExit
    end
  end
  object TimerPicker: TTimer
    Enabled = False
    Interval = 100
    OnTimer = TimerPickerTimer
    Left = 152
    Top = 144
  end
end
